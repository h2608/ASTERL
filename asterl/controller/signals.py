...
class SignalTracker:
    ...
    def __init__(self, pop_size, window_k, improve_eps, s_max,
        ...
        self.global_best = -np.inf  # raw single-episode record (champion bookkeeping)
        # Best window mean at the last gate reset. Improvement is measured
        # cumulatively against this fixed reference, not against the running
        # best: a sequence of sub-epsilon gains must still reset the gate once
        # it adds up past epsilon.
        self.gate_ref = -np.inf
        self.last_improve_step = 0
        # (env_steps, best window mean) samples for the trailing-window
        # progress gate; pruned to the window in gate_progress().
        self.curve = deque()
        self.peak_delta = 0.0

    def record_eval(self, idx, fitness, env_steps):
        """Returns (personal_improved, gate_reset)."""
        fitness = float(fitness)
        if not np.isfinite(fitness):
            raise ValueError(f"non-finite fitness {fitness!r} for individual {idx}")
        self.hist[idx].append(fitness)
        personal = fitness > self.personal_best[idx]
        if personal:
            self.personal_best[idx] = fitness
        if fitness > self.global_best:
            self.global_best = fitness
        # Gate signals run on the best *window mean* — the same statistic the
        # allocator ranks. One lucky episode moves it by at most 1/window_k
        # and decays back out of the window, instead of permanently raising a
        # running max that later real progress can never beat.
        best_mean = max(np.mean(h) for h in self.hist if len(h))
        if np.isfinite(self.gate_ref):
            improved = best_mean > self.gate_ref + self.improve_eps * max(
                abs(self.gate_ref), 1.0
            )
        else:
            # first finite signal: a -inf reference plus a relative margin is
            # nan and compares False, so it must be handled explicitly
            improved = True
        if improved:
            self.gate_ref = best_mean
            self.last_improve_step = env_steps - self.improve_decay * (
                env_steps - self.last_improve_step
            )
        self.curve.append((env_steps, best_mean))
        return personal, improved
    ...
    def gate_progress(self, env_steps):
        """Diminishing-marginal-return gate: 1 - (best-window-mean improvement
        over the trailing s_max steps) / (largest such window improvement this
        run). A declining mean reads as zero marginal return (delta clips at 0).
        ...
        while len(self.curve) >= 2 and self.curve[1][0] <= env_steps - self.s_max:
            self.curve.popleft()
        delta = self.curve[-1][1] - self.curve[0][1]
    ...
    def load_state_dict(self, d):
        ...
        self.gate_ref = d.get("gate_ref", -np.inf)
