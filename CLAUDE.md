...
- `asterl/common/` — buffer (float32, checkpointable), evaluator (gymnasium 5-tuple; **bootstrap mask = terminated only**), config (dataclass + YAML layering + `--set k=v`), logger (always writes `metrics.jsonl`; W&B/TensorBoard optional), checkpoint (atomic write), seeding (full RNG capture incl. env RNGs → bit-exact resume on CPU).
